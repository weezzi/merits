package com.merits.modules.base.dao;

// IMPORTS ---------------------------------------------------------------------

import java.sql.CallableStatement;
import java.sql.Timestamp;

import com.weezzi.core.data.*;
import com.weezzi.core.dbmanager.dao.UpdaterDAO;
import com.weezzi.utils.StringUtilities;


/**
 * <b>Title: </b>Good Type Updater Object<br/>
 * <b>Description: </b>Good Type Updater Class<br/>
 *
 * @author AUTO-GENERATED By Weezzi Builder
 * @version 1.0
 */
 @SuppressWarnings("unused")
public class GoodTypesUpdater implements UpdaterDAO {


    // PROPERTIES --------------------------------------------------------------

    private long id;
    private String name;
    private long orderNumber;
    private EditInfo editInfo;
    private String langCode;


    // CONSTRUCTORS ------------------------------------------------------------


	/**
	 * Empty Constructor
	 **/
	public GoodTypesUpdater(long id, String name, String langCode, UpdateInfo updateInfo) {
        this.id = id;
        this.name = name;
        this.orderNumber = orderNumber;
        this.editInfo = editInfo;
        this.langCode = langCode;
	}


	// PUBLIC -----------------------------------------------------------------

	
    /**
     * Gets DB Connection name
     * @return project database connection name;
     */
	public String getDBConnectionName() {
		return "Merits_Merits";
	}


	/**
	 * Gets update SQL
	 * @return query
	 */
	public String getQuerySQL() {
		return "UPDATE GOOD_TYPES, `weezziserver`.`MULTILANG_TRANSLATIONS` " +
               " SET `weezziserver`.MULTILANG_TRANSLATIONS.SMALL_TEXT1 = ? , ORDER_NUMBER = ? , EDIT_INFO_UPDATE_DATE  = ? , EDIT_INFO_UPDATE_USER  = ?  " +
               " WHERE GOOD_TYPES.MLK_ID = `weezziserver`.MULTILANG_TRANSLATIONS.ID_MLK AND `weezziserver`.MULTILANG_TRANSLATIONS.LANG_CODE = ? AND GOOD_TYPES.ID = ?";
	}
	
	
	/**
	 * Updates Query Fields
	 * @param statment
	 * @throws Exception
	 */
	public void updateFields(CallableStatement statement) throws Exception {
        // Set Fields
        statement.setString(1, name);
        statement.setLong(2, orderNumber);
        // Check if edit info create date is Null
        if (editInfo.getCreateDate() == null) {
            statement.setNull(3, java.sql.Types.TIMESTAMP);
        } else {
            // Set create date
            statement.setTimestamp(3,  editInfo.getCreateDate());
        } 
        // Check if edit info create user is Null
        if (editInfo.getCreateUserId() == null) {
            statement.setNull(4, java.sql.Types.NUMERIC);
        } else {
            // Set create user
            statement.setLong(4, editInfo.getCreateUserId());
        } 
        // Check if edit info update date is Null
        if (editInfo.getUpdateDate() == null) {
            statement.setNull(5, java.sql.Types.TIMESTAMP);
        } else {
            // Set update date
            statement.setTimestamp(5,  editInfo.getUpdateDate());
        } 
        // Check if edit info update user is Null
        if (editInfo.getUpdateUserId() == null) {
            statement.setNull(6, java.sql.Types.NUMERIC);
        } else {
            // Set update user
            statement.setLong(6, editInfo.getUpdateUserId());
        } 
        // Set multilang language code
        statement.setString(7, langCode);
        statement.setLong(8, id);
	}


	/**
	 * Gets extended update SQL
	 * @return query
	 */
	public String getExtendedQuerySQL() {
		return " UPDATE GOOD_TYPES SET MLK_LANGS = (" + 
                "SELECT group_concat(lang_code) FROM (SELECT MLK_ID FROM GOOD_TYPES WHERE ID=?) AS TEMP, `weezziserver`.multilang_translations " + 
                "WHERE `weezziserver`.multilang_translations.id_mlk= TEMP.MLK_ID " + 
                    "AND length(`weezziserver`.multilang_translations.SMALL_TEXT1) > 0 " + 
                ") WHERE ID = ?";
	}
	
	
	/**
	 * Updates Extended Query Fields
	 * @param statment
	 * @throws Exception
	 */
	public void updateExtendedQueryFields(CallableStatement statement) throws Exception {
        statement.setLong(1, id);
        statement.setLong(2, id);
	}
	
	
}
